...
class Solution14(str: String) extends Solution {
  ...
  def calculateValue(currentMask: String, number: Long): Long = {
    ...
    val newValue = numberBin.zip(currentMask).map(x => resolveMask(x._1, x._2)).mkString("")
    java.lang.Long.parseLong(newValue.toString, 2)
  }

  def firstPart(): Long = {
    val memoryMap: collection.mutable.Map[Int, Long] = collection.mutable.Map(0 -> 0)
    val reg = "[0-9]+".r
  ...
  def generateAddresses(floatMask: String): Array[Long] = {
    ...
    val subValues = range.map(_.toBinaryString.reverse.padTo(amountOfFloats.toBinaryString.length - 1, '0').reverse).toArray
    subValues.map(x => substituteFloats(x, floatMask)).map(x => java.lang.Long.parseLong(x.toString, 2))
  }

  def calculateAddresses(currentMask: String, address: Long): Array[Long] = {
    val floatMask = currentMask.zip(address.toBinaryString.reverse.padTo(36, '0').reverse)
      .map(x => resolveFloatMask(x)).toArray
    generateAddresses(floatMask.mkString(""))
  }

  def secondPart(): Long = {
    val memoryMap: collection.mutable.Map[Long, Long] = collection.mutable.Map(0L -> 0L)
    val reg = "[0-9]+".r
    var currentMask: String = ""
    val data = input.map(_.split(" = "))
    for (operation <- data){
